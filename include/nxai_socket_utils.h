 ...
 * If the socket file already existed at the specified path, it is removed before creating a new
 * one. The function sets a timeout for receiving data on the socket.
 *
 ...
 *
 * This function reads a message header from the socket, which specifies the full length of the
 * message. If the incoming message is larger than the allocated buffer size, or if the input
 * buffer is NULL, the function reallocates the buffer to fit the message. It then reads the
 * message into the buffer. If an error occurs during socket reading, a warning message is printed
 * to stderr.
 *
 ...
 * @param message_input_buffer A pointer to the input buffer in which to store the received
 * message.
 * @param message_length A pointer to a variable in which to store the length of the received
 * message.
 */
 ...
 * the buffer. If there is an error when receiving the socket message, a warning message is
 * printed.
 *
 * \param[in] socket_fd The file descriptor of the socket to await the message from.
 * \param[in,out] allocated_buffer_size The size of the allocated buffer. This value will be
 ...
 * \retval connection_fd An error occurred while the message header length does not equal
 * MESSAGE_HEADER_LENGTH or while reallocating the buffer.
 */
 ...
 * @param return_message_buffer Pointer to buffer that will hold the received message. Can be NULL
 * or a reusable buffer.
 * @param allocated_message_length Pointer to how much space in the existing buffer is allocated.
 * If too small, or `return_message_buffer` is NULL, `return_message_buffer` will be reallocated
 * and this variable updated.
 ...
 *
 * This function sends a message to a given socket. The message to send and its length are also
 * parameters of the function. It first sets the socket options, then sends the length of the
 * message as a header and finally sends the actual message. If any of the send operations fail,
