...
{
    bidirectional_pipe_t created_pipe = {
        {up_pipe_read, up_pipe_write},
        {down_pipe_read, down_pipe_write}};
    return created_pipe;
}
...
bidirectional_pipe_t nxai_create_pipe(int* error)
{
    ...
    // Create UP pipe
    bool success = nxai_create_pipe_handles(
        &(created_pipe.up_pipe[0]->handle),
        &(created_pipe.up_pipe[1]->handle));
    if (success == false)
    {
    ...
    // Create DOWN pipe
    success = nxai_create_pipe_handles(
        &(created_pipe.down_pipe[0]->handle),
        &(created_pipe.down_pipe[1]->handle));
    if (success == false)
    {
        nxai_vlog("Error! Couldn't create up pipe handles!\n");
        *error = -2;
    ...
    int8_t* return_byte)
{
    ...
    for (size_t pipe_index = 0; pipe_index < pipes_length; pipe_index++)
    {
        ...
        if (read_pipe->pending == true)
        {
            ...
            DWORD bytesRead = 0;
            if (GetOverlappedResult(read_pipe->handle, &(read_pipe->overlap), &bytesRead, TRUE))
            {
                if (bytesRead > 0)
                {
            ...
            // If there are more pending pipes they will be picked up next time this function is
            // called
            return pipe_index;
        }
    }
    ...
    for (size_t pipe_index = 0; pipe_index < pipes_length; pipe_index++)
    {
                ...
                &(read_pipe->overlap)))
        {
            ...
            if (lastError != ERROR_IO_PENDING)
            {
                ...
                {
                    CloseHandle(
                        nxai_pipe_get_read_pipe(pipes_array[j], direction)->overlap.hEvent);
                }
                free(handles);
                raise(SIGABRT);
                return pipes_length;
    ...
    DWORD result = WaitForMultipleObjects(pipes_length, handles, FALSE, timeout_ms);
    ...
    else if (result >= WAIT_OBJECT_0 && result < WAIT_OBJECT_0 + pipes_length)
    {
        ...
        {
            nxai_pipe_t check_pipe =
                nxai_pipe_get_read_pipe(pipes_array[completed_index], direction);
        ...
        nxai_pipe_t completed_pipe =
            nxai_pipe_get_read_pipe(pipes_array[completed_index], direction);
        completed_pipe->active = false;
        // Get the result of the completed operation
        DWORD bytesRead = 0;
        if (GetOverlappedResult(
                completed_pipe->handle,
...
char nxai_pipe_read(bidirectional_pipe_t pipe_fd, PIPE_DIRECTION direction)
{
    ...
    DWORD bytes_read;
    if (!ReadFile(
            nxai_pipe_get_read_pipe(pipe_fd, direction)->handle,
            &buffer,
            1,
            &bytes_read,
            NULL))
...
ssize_t nxai_pipe_send(bidirectional_pipe_t pipe_fd, PIPE_DIRECTION direction, char signal)
{
    ...
    DWORD bytes_written;
    if (!WriteFile(
            nxai_pipe_get_write_pipe(pipe_fd, direction)->handle,
            &signal,
            1,
            &bytes_written,
            NULL))
...
char* nxai_read_pipe_to_string(nxai_pipe_t pipe)
{
    ...
    while (true)
    {
        ...
        BOOL success = ReadFile(pipe->handle, buffer, sizeof(buffer), &bytes_read, &ov);
